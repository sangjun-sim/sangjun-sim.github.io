...
<!-- REVIEW: [low] The bar on \bar{g}_x is redundant. The exchange part is evaluated from the (\lambda-independent) Kohn-Sham determinant, so \int_0^1 d\lambda~g_{\mathrm{x},\lambda} = g_{\rm x}. Only the correlation part genuinely depends on \lambda, and \bar{g}_{\rm c} differs from the physical g_{\rm c}(\lambda = 1) precisely by the kinetic correlation contribution mentioned above. -->
